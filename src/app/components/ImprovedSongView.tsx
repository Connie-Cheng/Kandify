...
import { X, Check, MessageSquare, Play, Pause, Upload } from 'lucide-react';
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type.startsWith('image/')) {
      ...
      reader.onload = (event) => {
        ...
        // Auto-select this new image bead
        setSelectedBead({
        ...
        <div className="flex-1 overflow-y-auto p-5 scrollbar-thin">
          <div className="max-w-4xl mx-auto">
            <div className="glass-card rounded-3xl p-6 inner-glow vignette relative">
              {/* Persistent Cord Line - Centered under beads */}
              <div
              ...
              <div className="space-y-3 relative z-10">
                {lyrics.map((lyric, index) => {
                  ...
                  let opacity = distance === 0 ? 1 : distance === 1 ? 0.7 : distance === 2 ? 0.5 : distance === 3 ? 0.35 : 0.2;
                  let blur = distance === 0 ? 0 : distance === 1 ? 0.5 : distance === 2 ? 1 : 1.5;
                  ...
                  return (
                    <div key={index} className="flex items-start gap-4 group">
                      ...
                      <div className="flex-1">
                        ...
                        >
                          <p className={`text-2xl leading-relaxed ${isActive ? 'font-bold text-white' : 'font-medium text-white/80'}`}>
                            {lyric.text}
                          </p>
                        </div>
              ...
              <p className="text-center text-xs text-white/40 mt-6">
                {selectedBead ? 'Double-click lyric to add bead • Double-click bead to remove • Click lyric to seek' : '👆 Select a bead from the left panel'}
              </p>
            </div>
          </div>
        </div>
